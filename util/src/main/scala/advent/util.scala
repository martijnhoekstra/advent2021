...
extension (name: String)
  def bytes[F[_]: Sync]: Stream[F, Byte] =
    def resource = Option(getClass().getResourceAsStream(name)).getOrElse(throw new IOException(s"resource $name not found"))
    io.readInputStream(Sync[F].delay(resource), 256, true)
  def resourceLines[F[_]: Sync]: Stream[F, String] = bytes.through(text.utf8.decode).through(text.lines)
